            ...
            while($row = $rs2->fetch_assoc()) {
                ...
                if($flag==1){
                    if(trim($f_feedback)=='' or trim($f_total_students)==''){
                        $sql = "UPDATE `faculty_details` SET `f_feedback`='$faculty_res',`f_total_students`= 1 ,crt_dttm=CURRENT_TIMESTAMP WHERE f_id = $f_id";
                        ...
                        $sql4 = "UPDATE `faculty_details` SET `f_feedback`='$new_response', `f_total_students`= ".(((int)$f_total_students)+1).",crt_dttm=CURRENT_TIMESTAMP WHERE f_id = $f_id";
                        echo $sql4;
                        $cn->query($sql4);
