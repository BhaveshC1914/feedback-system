...
if(isset($_POST['submit_feedback'])){
    ...
    if($rs->num_rows > 0 ){
        ...
        for($i=1;$i<=1;$i++){
            ...
            $response_c=$response_c.$int_res;
        }
        $sql2 = "SELECT f_id FROM faculty_details where f_sem='".$_SESSION['s_sem']."' and f_branch='".$_SESSION['s_branch']."'";
        $rs2 = $cn->query($sql2);
        if ($rs2->num_rows > 0) {
            while($row = $rs2->fetch_assoc()) {
                ...
                for($j=1;$j<=10;$j++){
                    ...
                    if(addslashes($_POST["f".$f_id."q".$j]) != ''){
                        $faculty_res=$faculty_res."f".$f_id."q".$j.':'.addslashes($_POST["f".$f_id."q".$j]).'|';
                    }else{
                        $flag=0;
                    }
                }
                $response_f=$response_f.$faculty_res;
            }
        }
        print($response_c);
        print($response_f);
        ...
        if($flag==1){
            $sql = "INSERT INTO `feedback_details` VALUES (".$_SESSION["s_id"].", '$response_f','$response_c', NULL)";
